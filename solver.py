...
def trf_linear(A, b, x_lsq, lb, ub, tol, max_iter,
               verbose):
    m, n = A.shape
    ...
    for iteration in range(max_iter):
        ...
        #p_h = -linalg.spsolve(A.astype("double"), b)

        #lsmr_op = regularized_lsq_operator(A_h, diag_root_h)
        #r_aug[:m] = r

        #p_h = -lsmr(lsmr_op, r_aug, atol=lsmr_tol, btol=lsmr_tol)[0]
        #p_h = - linalg.spsolve(lsmr_op.astype("double"), r_aug)
        #p_h = -linalg.spsolve(A.astype("double"), d)
